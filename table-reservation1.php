	...
	<section class="table_reservation_section">
        ...
        <div class="container">
            ...
            if(isset($_POST['submit_table_reservation_form']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            {
                ...
                try
                {
                    $stmtgetCurrentClientID = $con->prepare("SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'kamp' AND TABLE_NAME = 'clients'");
                    ...
                    $stmt_reservation = $con->prepare("insert into reservations(date_created, client_id, selected_time, nbr_guests, table_number) values(?, ?, ?, ?, ?)");
                    $stmt_reservation->execute(array(Date("Y-m-d H:i"),$client_id[0], $desired_date, $number_of_guests, $table_number));
...
<div class="row">
        <div id="fp-canvas-container">
            <img src="./Design/images/floorplan.png" alt="Floor Plan" class='fp-img border p-1' id="fp-img" usemap="#fp-map">
            ...
            <form method="POST" action="table-reservation1.php">
                <div class="row">
                    ...
                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
                        <div class="form-group">
                            <label for="table_number">Choose Table</label>
                            <select class="form-control" name="table_number">
                                <option value="1" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>
                                    Camp Awning 1
                                </option>
                                <option value="2" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Center 2</option>
                                <option value="3" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Center 3</option>
                                <option value="4" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Center 4</option>
                                <option value="4" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Center 5</option>
                                <option value="4" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Tent 1</option>
                                <option value="4" <?php echo (isset($_POST['table_number']))?"selected":"";  ?>>Tent 2</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-6">
            ...
            <?php
                ...
                if(isset($_POST['check_availability_submit']))
                {
                    ...
                    $stmt = $con->prepare("select table_number
                        from tables

                        where table_number not in (select t.table_number
